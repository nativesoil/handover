 ...
 * block, which `soil save --stdin` (or `--from-file`) ingests as a real
 * handover. It is the manual on-ramp, and it is the reason the format has to be
 * writable by a model with no tools at all.
 *
 * Embedded verbatim from Native Soil's production rescue prompt, with one word
 * changed: the provenance line says "never present a guess as checked" rather
 * than the word for independently confirmed, because nothing in this repo
 * confirms anything.
 *
 * The block it asks for uses the loose shape (`extractionSections`, plain
 * strings). `normalizeHandover` turns that into a spec-shaped document, which
 * is how the manual on-ramp reaches the same store as everything else.
 *
 * THE GAP THAT WAS HERE, now closed. This prompt used to ask for exactly
 * `projectId`, `title` and `extractionSections`, and never for `createdAt`.
 * Normalization used to fill that in from the wall clock, which made the
 * capture time of every rescued handover a fabrication, so it no longer does:
 * a document with no `createdAt` is refused. That left this prompt producing
 * replies that could not validate, so it now asks the model for the timestamp
 * as well. It also asks for `source.recipeVersion`, for the same reason in the
 * other direction: no writer is in a position to attest which recipe produced a
 * document, and the model reading this text is, so the version is printed on
 * the first line above and the model reports it back.
 *
 * Pure string, one dependency: the recipe version it prints.
 */

import { RECIPE_VERSION } from "./recipe.js";
export const RESCUE_PROMPT = `SOIL RESCUE SAVE — spec v1.0 · recipe v${RECIPE_VERSION}
